 ...
 */
export async function downloadWorkdriveFile(fileId, isRetry = false) {
    ...
    if (!res.ok) {
        ...
        throw new HttpError(res.status >= 500 ? 502 : res.status, `Zoho WorkDrive download request failed: HTTP ${res.status} ${res.statusText}`);
    }
    return res;
}
