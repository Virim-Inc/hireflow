...
async function getCandidateById(client, candidateId) {
    const result = await client.query(`SELECT ${SELECT_COLUMNS}
     FROM candidates
     WHERE id = $1`, [candidateId]);
    return result.rows[0] ?? null;
}
function parseCandidateId(rawId) {
...
app.get('/api/candidates', async (req, res) => {
    try {
        ...
        const countRes = await pool.query(`SELECT COUNT(*) FROM candidates ${whereClause}`, params);
        const total = Number.parseInt(countRes.rows[0].count, 10);
        const dataRes = await pool.query(`SELECT ${SELECT_COLUMNS}
       FROM candidates
       ${whereClause}
...
app.patch('/api/candidates/:id/stage', async (req, res) => {
    ...
    try {
        ...
        const current = await client.query('SELECT pipeline_stage FROM candidates WHERE id = $1 FOR UPDATE', [candidateId]);
        ...
        await client.query(`UPDATE candidates
         SET pipeline_stage = $2,
             ...
             latest_stage_note = $3
         WHERE id = $1`, [candidateId, stage, trimmedNote]);
        await client.query(`INSERT INTO candidate_stage_history (candidate_id, from_stage, to_stage, note)
         VALUES ($1, $2, $3, $4)`, [candidateId, previousStage, stage, trimmedNote]);
