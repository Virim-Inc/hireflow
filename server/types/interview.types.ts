// server/types/interview.types.ts
...
export type AvailabilityRequestStatus = 'pending' | 'accepted' | 'reschedule_proposed' | 'declined' | 'expired' | 'consumed' | 'revoked';
...
export interface InterviewRow {
  ...
  alternative_slots?: InterviewAvailabilitySlotRow[];
  feedback?: InterviewFeedbackRow[];
  events?: InterviewEventRow[];
}

export interface InterviewParticipantRow {
  id: number;
  interview_id: number;
  user_id: number | null;
  name: string;
...
export interface InterviewAvailabilityRequestRow {
  id: number;
  interview_id: number;
...
export interface InterviewAvailabilitySlotRow {
  id: number;
